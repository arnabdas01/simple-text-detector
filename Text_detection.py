...
img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)              #changing image from BRG to RGB colorspace
...
for i, a in enumerate(boxes.splitlines()):
    if i != 0:                                          #the first row of boxes is ignored i.e the heading row
        a = a.split()                                   
        # print(a)
        if len(a) == 12:                                #these rows contains the actual words present in the image
            x, y, w, h = int(a[6]), int(a[7]), int(a[8]), int(a[9])
            cv2.rectangle(img, (x, y), (w + x, h + y), (0, 0, 255), 1)                          #rectangle around words
